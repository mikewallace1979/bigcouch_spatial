...
Create a database:
    ...
    curl -X PUT http://127.0.0.1:15984/places

Add a Design Document with a spatial function:

    curl -X PUT -d '{"spatial":{"points":"function(doc) {\n    if (doc.loc) {\n        emit({\n            type: \"Point\",\n            coordinates: [doc.loc[0], doc.loc[1]]\n        }, [doc._id, doc.loc]);\n    }};"}}' http://127.0.0.1:15984/places/_design/main

Put some data into it:

    curl -X PUT -d '{"loc": [-122.270833, 37.804444]}' http://127.0.0.1:15984/places/oakland
    curl -X PUT -d '{"loc": [10.898333, 48.371667]}' http://127.0.0.1:15984/places/augsburg

Make a bounding box request:

    curl -X GET 'http://localhost:15984/places/_design/main/_spatial/points?bbox=0,0,180,90'
...
above). Insert some Documents:
    ...
    curl -X PUT -d '{"loc": [17.15, -22.566667]}' http://127.0.0.1:15984/places/namibia
    curl -X PUT -d '{"loc": [135, -25]}' http://127.0.0.1:15984/places/australia
    curl -X PUT -d '{"loc": [-52.95, -10.65]}' http://127.0.0.1:15984/places/brasilia

And request only Australia and Brasilia:

    curl -X GET 'http://localhost:15984/places/_design/main/_spatial/points?bbox=110,-60,-30,15&plane_bounds=-180,-90,180,90'
...
(`-30,-60,110,15`) would only return Namibia:
    ...
    curl -X GET 'http://localhost:15984/places/_design/main/_spatial/points?bbox=-30,-60,110,15&plane_bounds=-180,-90,180,90'
...
`_list` ):
    ...
    curl -X GET 'http://localhost:15984/places/_design/main/_spatial/_list/wkt/points?bbox=-180,-90,180,90'
...
ID in parenthesis:
    ...
    curl -X PUT -d '{"lists": {"wkt": "function(head, req) {\n    var row;\n    while (row = getRow()) {\n        send(\"POINT(\" + row.value[1].join(\" \") + \") (\" + row.id + \")\\n\");\n    }\n};"}}' http://127.0.0.1:15984/places/_design/listfunonly

    curl -X GET 'http://localhost:15984/places/_design/listfunonly/_spatial/_list/wkt/main/points?bbox=-180,-90,180,90'
...
To cleanup spatial indexes that are no longer in use (this is per database):
    ...
    curl -X POST 'http://localhost:15984/places/_spatial_cleanup' -H 'Content-Type: application/json'

To get information about the spatial indexes of a certain Design
Document use the the `_info` handler:

    curl -X GET 'http://localhost:15984/places/_design/main/_spatial/_info'
